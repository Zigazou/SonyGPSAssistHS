...
main = do
    ...
    sonyMounts <- actionIO "Analyze mounted filesystems" getSonyMounts

    -- Download GPS data from Sony site using Curl
    gpsData <- actionIO "Retrieve GPS data from Sony site"
                        (httpGetBin gpsDataUrl)

    -- Download corresponding MD5 file
    gpsMD5raw <- actionIO "Retrieve MD5 for GPS data from Sony site"
                          (httpGetString gpsMD5Url)

    -- Verify the MD5 matches
    action "Check MD5" (verifyMD5 gpsData $ take 32 gpsMD5raw)

    -- For any Sony mounted storage device found, copy the GPS data
    forM_ sonyMounts $ \mDir -> actionIO ("Write GPS data on " ++ mDir)
